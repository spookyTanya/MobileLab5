...
public class Renderer implements GLSurfaceView.Renderer
{
	...
	private final FloatBuffer mVertices2;
...
//				0, -1,0,
		...
		// Initialize the buffers. виділення пам'яті на буфер
		mVertices1 = ByteBuffer.allocateDirect(mVerticesData1.length * mBytesPerFloat)
        .order(ByteOrder.nativeOrder()).asFloatBuffer();

		mVertices1.put(mVerticesData1).position(0);

		mVertices2 = ByteBuffer.allocateDirect(mVerticesData2.length * mBytesPerFloat)
        .order(ByteOrder.nativeOrder()).asFloatBuffer();
	...
	public void onSurfaceCreated(GL10 glUnused, EGLConfig config)
	{
		...
		// позиція фігури на екрані і точка з якої ми будемо на неї дивитись
	...
	public void onDrawFrame(GL10 glUnused)
	{
        ...
        long time = SystemClock.uptimeMillis() % 10000000L;
        float angleInDegrees = (360.0f / 10000000.0f) * ((int) time);

        // Draw the triangle facing straight on.
        Matrix.setIdentityM(mModelMatrix, 0);
       // Matrix.rotateM(mModelMatrix, 0, 0, 0.0f, 0.0f, 1.0f);
//		drawTriangleStrip(mVertices, 11);
		drawTriangleStrip(mVertices1, 4);
		drawTriangleStrip(mVertices2, 4);
...
//		GLES20.glUniform4f(mColorHandle, 0.3f, 0.8f, 0.3f, 1.0f);
		//колір фігури
		GLES20.glUniform4f(mColorHandle, 0.8f, 0.3f, 0.1f, 1.0f);

//        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, num_vertices);
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_FAN, 0, num_vertices);

        GLES20.glDisable(GLES20.GL_DEPTH_TEST);
        GLES20.glEnable(GLES20.GL_BLEND);
		GLES20.glBlendFunc(GLES20.GL_ONE, GLES20.GL_ONE);
        GLES20.glBlendEquation(GLES20.GL_FUNC_ADD);
		GLES20.glUniform4f(mColorHandle, 0.2f, 0.8f, 0.8f, 1.0f);
